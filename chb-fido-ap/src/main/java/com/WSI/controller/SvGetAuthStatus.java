...
/**
 * Servlet implementation class svGetAuthStatus
 */
@RestController
// @WebServlet(description = "取得交易驗證狀態", urlPatterns = { "/svGetAuthStatus" })
public class SvGetAuthStatus extends BaseServlet {
	@Autowired
	WSMServlet wsmServlet;

	@PostMapping("/svGetAuthStatus")
	protected String doPost(@RequestBody Object reqBody, HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		inLogObj.setClazz("com.WSI.controller.svGetAuthStatus");
		apLogObj.setTraceID(sessionID);
		apLogObj.setClazz("com.WSI.controller.svGetAuthStatus");
...
//				+ ", Err_msg_path: " + Err_msg_path);
		...
		Log4jWSI.log.debug(
				"[" + sessionID + "][Version: " + svVerNo + "][svGetAuthStatus] API key in header: " + headerApiKey);
		apLogObj.setMessage(
				"[" + sessionID + "][Version: " + svVerNo + "][svGetAuthStatus] API key in header: " + headerApiKey);
		...
		if (headerApiKey == null || !WSI_API_Key.equals(headerApiKey)) {
			Log4jWSI.log.debug("[" + sessionID + "][Version: " + svVerNo
					+ "][svGetAuthStatus] Reject client request for invalid API key");
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.APIKeyInvalid));
		...
		try {
			...
			Log4jWSI.log.fatal("[" + sessionID + "][Version: " + svVerNo
					+ "][svGetAuthStatus] Error occurred while reading body: " + e.getMessage());
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.InternalError));
		...
		Log4jWSI.log.info("[" + sessionID + "][Version: " + svVerNo + "][svGetAuthStatus] Msg from body: " + jsonString);

		apLogObj.setMessage(
				"[" + sessionID + "][Version: " + svVerNo + "][svGetAuthStatus] Msg from body: " + jsonString);
		...
		try {
			...
			Log4jWSI.log.error(
					"[" + sessionID + "][Version: " + svVerNo + "][svGetAuthStatus] Unable to parse client JSON msg: "
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.JsonParseError));
		...
		// log locale
		Log4jWSI.log.trace("[" + sessionID + "][Version: " + svVerNo + "][svGetAuthStatus] Locale: " + req.getLocale());

		apLogObj.setMessage(
				"[" + sessionID + "][Version: " + svVerNo + "][svGetAuthStatus] Locale: " + req.getLocale());
		...
		if (!map.containsKey("Error")) {
			...
			Log4jWSI.log.info("[" + sessionID + "][Version: " + svVerNo + "][svGetAuthStatus] channel: " + channel
					+ ", idgateID: " + idgateID + ", txnID: " + txnID);

			apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo + "][svGetAuthStatus] channel: " + channel
					+ ", idgateID: " + idgateID + ", txnID: " + txnID);
			Log4jAP.log.info(apLogObj.getCompleteTxt());
		} else {
			Log4jWSI.log.info("[" + sessionID + "][Version: " + svVerNo + "][svGetAuthStatus] Error: " + map.get("Error")
...
//						.OutResult(gts.common(ReturnCode.ChannelInvalidError));
			...
			if (memVO == null) {
				Log4jWSI.log.info("[" + sessionID + "][Version: " + svVerNo
						+ "][svGetAuthStatus] Member not found. Response: " + gts.common(ReturnCode.MemberNotFound));
...
//						.OutResult(gts.common(ReturnCode.MemberNotFound));
			...
			Log4jWSI.log.error("[" + sessionID + "][Version: " + svVerNo + "][svGetAuthStatus] DB Error occurred: "
					+ e.getMessage() + ", stacktrace: " + new Gson().toJson(e.getStackTrace()));

			apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo + "][svGetAuthStatus] DB Error occurred: "
					+ e.getMessage());
			apLogObj.setThrowable(e.getMessage());
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.DatabaseError));
		...
		Log4jWSI.log.trace(
				"[" + sessionID + "][Version: " + svVerNo + "][svGetAuthStatus] Sending to FIDO svGetAuthStatus API");
		apLogObj.setMessage(
				"[" + sessionID + "][Version: " + svVerNo + "][svGetAuthStatus] Sending to FIDO svGetAuthStatus API");
		Log4jAP.log.info(apLogObj.getCompleteTxt());
		...
		formparams.put("method", "svGetAuthStatus");
...
//			retMsg = new Send2Remote().sendPost(FIDO_SV_IP, WSI_API_Key, gson.toJson(formparams));
			...
			Log4jWSI.log.error("[" + sessionID + "][Version: " + svVerNo + "][svGetAuthStatus] Connection to FIDO error: "
					+ e.getMessage() + ", stacktrace: " + new Gson().toJson(e.getStackTrace()));

			apLogObj.setMessage(
					"[" + sessionID + "][Version: " + svVerNo + "][svGetAuthStatus] Connection to FIDO error: "
							+ e.getMessage() + ", stacktrace: " + new Gson().toJson(e.getStackTrace()));
			apLogObj.setThrowable(e.getMessage());
			Log4jAP.log.error(apLogObj.getCompleteTxt());
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.FidoSVError));
		...
		Log4jWSI.log.info("[" + sessionID + "][Version: " + svVerNo + "][svGetAuthStatus] FIDO SV Response: " + retMsg);

		apLogObj.setMessage(
				"[" + sessionID + "][Version: " + svVerNo + "][svGetAuthStatus] FIDO SV Response: " + retMsg);
		Log4jAP.log.info(apLogObj.getCompleteTxt());
		...
		if (map.containsKey("Error")) {
			Log4jWSI.log
					.debug("[" + sessionID + "][Version: " + svVerNo + "][svGetAuthStatus] Error: " + map.get("Error"));
		...
		try {
			...
			Log4jWSI.log
					.error("[" + sessionID + "][Version: " + svVerNo + "][svGetAuthStatus] Unable to parse FIDO JSON msg: "
							+ e.getMessage() + ", stacktrace: " + new Gson().toJson(e.getStackTrace()));

			apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo
					+ "][svGetAuthStatus] Unable to parse FIDO JSON msg: " + e.getMessage());
			apLogObj.setThrowable(e.getMessage());
			Log4jAP.log.error(apLogObj.getCompleteTxt());
		...
		Log4jWSI.log.info("[" + sessionID + "][Version: " + svVerNo + "][svGetAuthStatus] Response: " + retMsg);
