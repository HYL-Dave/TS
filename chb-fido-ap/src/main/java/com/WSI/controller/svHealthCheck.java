...
/**
 * Servlet implementation class svHealthCheck
 */
@RestController
// @WebServlet(description = "查詢服務狀態", urlPatterns = { "/svHealthCheck" })
public class svHealthCheck extends BaseServlet {
	
	@Autowired
	WSMServlet wsmServlet;
	
	@PostMapping("/svHealthCheck")
	protected String doPost(@RequestBody Object reqBody, HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		inLogObj.setClazz("com.WSI.controller.svHealthCheck");
		apLogObj.setTraceID(sessionID);
		apLogObj.setClazz("com.WSI.controller.svHealthCheck");
...
//				+ ", Err_msg_path: " + Err_msg_path);
		...
		Log4jWSI.log.debug(
				"[" + sessionID + "][Version: " + svVerNo + "][svHealthCheck] API key in header: " + headerApiKey);
		apLogObj.setMessage(
				"[" + sessionID + "][Version: " + svVerNo + "][svHealthCheck] API key in header: " + headerApiKey);
		...
		if (headerApiKey == null || !WSI_API_Key.equals(headerApiKey)) {
			Log4jWSI.log.debug("[" + sessionID + "][Version: " + svVerNo
					+ "][svHealthCheck] Reject client request for invalid API key");
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.APIKeyInvalid));
		...
		try {
			...
			Log4jWSI.log.fatal("[" + sessionID + "][Version: " + svVerNo
					+ "][svHealthCheck] Error occurred while reading body: " + e.getMessage());
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.InternalError));
		...
		Log4jWSI.log.info("[" + sessionID + "][Version: " + svVerNo + "][svHealthCheck] Msg from body: " + jsonString);

		apLogObj.setMessage(
				"[" + sessionID + "][Version: " + svVerNo + "][svHealthCheck] Msg from body: " + jsonString);
		...
		try {
			...
			Log4jWSI.log.error(
					"[" + sessionID + "][Version: " + svVerNo + "][svHealthCheck] Unable to parse client JSON msg: "
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.JsonParseError));
		...
		// log locale
		Log4jWSI.log.trace("[" + sessionID + "][Version: " + svVerNo + "][svHealthCheck] Locale: " + req.getLocale());

		apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo + "][svHealthCheck] Locale: " + req.getLocale());
		...
		if (!map.containsKey("Error")) {
			...
			Log4jWSI.log.info("[" + sessionID + "][Version: " + svVerNo + "][svHealthCheck] channel: " + channel);

			apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo + "][svHealthCheck] channel: " + channel);
			Log4jAP.log.info(apLogObj.getCompleteTxt());

		} else {
			Log4jWSI.log.info("[" + sessionID + "][Version: " + svVerNo + "][svHealthCheck] Error: " + map.get("Error")
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.ParameterError));
		...
		Log4jWSI.log.trace("[" + sessionID + "][Version: " + svVerNo + "][svHealthCheck] Testing DB...");
		ChannelService chSvc = new ChannelService(JNDI_Name, sessionID);

		try {
			// check if DB connection is ok
			chSvc.getOneChannel(channel);
		} catch (SQLException e) {
			Log4jWSI.log.error("[" + sessionID + "][Version: " + svVerNo + "][svHealthCheck] DB Error occurred: "
					+ e.getMessage() + ", stacktrace: " + new Gson().toJson(e.getStackTrace()) + ", Response: "
					+ gts.common(ReturnCode.DatabaseError));

			apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo + "][svHealthCheck] DB Error occurred: "
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.DatabaseError));
		...
		Log4jWSI.log.trace("[" + sessionID + "][Version: " + svVerNo + "][svHealthCheck] Testing FIDO SV...");
...
//			retMsg = new Send2Remote().sendPost(FIDO_SV_IP, WSI_API_Key, gson.toJson(formparams));
			...
			Log4jWSI.log.error("[" + sessionID + "][Version: " + svVerNo + "][svHealthCheck] Connection to FIDO error: "
					+ e.getMessage() + ", stacktrace: " + new Gson().toJson(e.getStackTrace()) + ", Response: "
					+ gts.common(ReturnCode.FidoSVError));

			apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo
					+ "][svHealthCheck] Connection to FIDO error: " + e.getMessage() + ", stacktrace: "
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.FidoSVError));
		...
		try {
			...
			Log4jWSI.log.error(
					"[" + sessionID + "][Version: " + svVerNo + "][svHealthCheck] Unable to parse FIDO JSON msg: "
							+ e.getMessage() + ", stacktrace: " + new Gson().toJson(e.getStackTrace()));

			apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo
					+ "][svHealthCheck] Unable to parse FIDO JSON msg: " + e.getMessage());
			apLogObj.setThrowable(e.getMessage());
			Log4jAP.log.error(apLogObj.getCompleteTxt());
		...
		Log4jWSI.log.info("[" + sessionID + "][Version: " + svVerNo + "][svHealthCheck] Response: " + retMsg);

		apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo + "][svHealthCheck] Response: " + retMsg);
		Log4jAP.log.info(apLogObj.getCompleteTxt());
