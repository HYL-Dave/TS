...
/**
 * Servlet implementation class SvfChangeSetting
 */
@RestController
// @WebServlet(description = "變更密碼設定", urlPatterns = { "/svfChangeSetting" })
public class SvfChangeSetting extends BaseServlet {
	

	@Autowired
	WSMServlet wsmServlet;
	
	@PostMapping("/svfChangeSetting")
	protected String doPost(@RequestBody Object reqBody, HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		inLogObj.setClazz("com.WSM.controller.SvfChangeSetting");
		apLogObj.setTraceID(sessionID);
		apLogObj.setClazz("com.WSM.controller.SvfChangeSetting");
...
//				+ ", Err_msg_path: " + Err_msg_path);
		...
		Log4j.log.debug(
				"[" + sessionID + "][Version: " + svVerNo + "][SvfChangeSetting] API key in header: " + headerApiKey);
		apLogObj.setMessage(
				"[" + sessionID + "][Version: " + svVerNo + "][SvfChangeSetting] API key in header: " + headerApiKey);
		...
		if (headerApiKey == null || !WSM_API_Key.equals(headerApiKey)) {
			Log4j.log.debug("[" + sessionID + "][Version: " + svVerNo
					+ "][SvfChangeSetting] Reject client request for invalid API key");
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.APIKeyInvalid));
		...
		try {
			...
			Log4j.log.fatal("[" + sessionID + "][Version: " + svVerNo
					+ "][SvfChangeSetting] Error occurred while reading body: " + e.getMessage());
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.InternalError));
		...
		Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][SvfChangeSetting] Msg from body: " + jsonString);

		apLogObj.setMessage(
				"[" + sessionID + "][Version: " + svVerNo + "][SvfChangeSetting] Msg from body: " + jsonString);
		...
		try {
			...
			map.put("changeSettingSession", jSONObject.getString("changeSettingSession"));
			map.put("encChangeSettingData", jSONObject.getString("encChangeSettingData"));

		} catch (JsonSyntaxException | JSONException e) {
			Log4j.log.error("[" + sessionID + "][Version: " + svVerNo
					+ "][SvfChangeSetting] Unable to parse client JSON msg: " + e.getMessage() + ", stacktrace: "
					+ new Gson().toJson(e.getStackTrace()) + ", Response: " + gts.common(ReturnCode.JsonParseError));

			apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo
					+ "][SvfChangeSetting] Error occurred while reading body: " + e.getMessage() + ", Response: "
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.JsonParseError));
		...
		// log locale
		Log4j.log.trace("[" + sessionID + "][Version: " + svVerNo + "][SvfChangeSetting] Locale: " + req.getLocale());

		apLogObj.setMessage(
				"[" + sessionID + "][Version: " + svVerNo + "][SvfChangeSetting] Locale: " + req.getLocale());
		Log4jAP.log.trace(apLogObj.getCompleteTxt());

		String channel;
		String idgateID;
		String changeSettingSession;
		String encChangeSettingData;
		if (!map.containsKey("Error")) {
			channel = map.get("channel");
			idgateID = map.get("idgateID");
			changeSettingSession = map.get("changeSettingSession");
			encChangeSettingData = map.get("encChangeSettingData");

			Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][SvfChangeSetting] channel: " + channel
					+ ", idgateID: " + idgateID + ", changeSettingSession: " + changeSettingSession
					+ ", encChangeSettingData: " + encChangeSettingData);

			apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo + "][SvfChangeSetting] channel: " + channel
					+ ", idgateID: " + idgateID + ", changeSettingSession: " + changeSettingSession
					+ ", encChangeSettingData: " + encChangeSettingData);
			Log4jAP.log.info(apLogObj.getCompleteTxt());

		} else {
			Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][SvfChangeSetting] Error: " + map.get("Error")
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.ParameterError));
		...
		try {
			...
			Log4j.log.error("[" + sessionID + "][Version: " + svVerNo + "][SvfChangeSetting] DB Error occurred: "
					+ e.getMessage() + ", stacktrace: " + new Gson().toJson(e.getStackTrace()));

			apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo + "][SvfChangeSetting] DB Error occurred: "
					+ e.getMessage());
			apLogObj.setThrowable(e.getMessage());
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.DatabaseError));
		...
		if (chVO == null || "N".equals(chVO.getACTIVATE())) {
			Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][SvfChangeSetting] Response: "
...
//					.OutResult(gts.common(ReturnCode.ChannelInvalidError));
		...
		Log4j.log.trace(
				"[" + sessionID + "][Version: " + svVerNo + "][svfChangeSetting] Sending to FIDO svfChangeSetting API");
		apLogObj.setMessage(
				"[" + sessionID + "][Version: " + svVerNo + "][svfChangeSetting] Sending to FIDO svfChangeSetting API");
		Log4jAP.log.info(apLogObj.getCompleteTxt());
		...
		formparams.put("method", "svfChangeSetting");
		formparams.put("channel", channel);
		formparams.put("idgateID", idgateID);
		formparams.put("changeSettingSession", changeSettingSession);
		formparams.put("encChangeSettingData", encChangeSettingData);
...
//			retMsg = new Send2Remote().sendPost(FIDO_SV_IP, WSM_API_Key, gson.toJson(formparams));
			...
			Log4j.log.error("[" + sessionID + "][Version: " + svVerNo + "][SvfChangeSetting] Connection to FIDO error: "
					+ e.getMessage() + ", stacktrace: " + new Gson().toJson(e.getStackTrace()));

			apLogObj.setMessage(
					"[" + sessionID + "][Version: " + svVerNo + "][SvfChangeSetting] Connection to FIDO error: "
							+ e.getMessage() + ", stacktrace: " + new Gson().toJson(e.getStackTrace()));
			apLogObj.setThrowable(e.getMessage());
			Log4jAP.log.error(apLogObj.getCompleteTxt());
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.FidoSVError));
		...
		Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][SvfChangeSetting] FIDO SV Response: " + retMsg);

		apLogObj.setMessage(
				"[" + sessionID + "][Version: " + svVerNo + "][SvfChangeSetting] FIDO SV Response: " + retMsg);
		Log4jAP.log.info(apLogObj.getCompleteTxt());
		...
		if (map.containsKey("Error")) {
			Log4j.log.debug(
					"[" + sessionID + "][Version: " + svVerNo + "][SvfChangeSetting] Error: " + map.get("Error"));
		...
		try {
			...
			Log4j.log.error(
					"[" + sessionID + "][Version: " + svVerNo + "][SvfChangeSetting] Unable to parse FIDO JSON msg: "
							+ e.getMessage() + ", stacktrace: " + new Gson().toJson(e.getStackTrace()));

			apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo
					+ "][SvfChangeSetting] Unable to parse FIDO JSON msg: " + e.getMessage());
			apLogObj.setThrowable(e.getMessage());
			Log4jAP.log.error(apLogObj.getCompleteTxt());
		...
		Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][SvfChangeSetting] Response: " + retMsg);
