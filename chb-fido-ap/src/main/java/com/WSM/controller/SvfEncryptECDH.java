...
/**
 * Servlet implementation class svfEncryptECDH
 */
//@RequestMapping("/MP")
// @WebServlet(description = "ECDH加密", urlPatterns = { "/svfEncryptECDH" })
@RestController
public class SvfEncryptECDH extends BaseServlet {
	...
	protected String doPost(@RequestBody Object body) throws ServletException, IOException {
		...
		Log4j.log.debug(
				"[" + sessionID + "][Version: " + svVerNo + "][svfEncryptECDH] *** getLocale: " + req.getLocale());

		Properties properties = null;
		File exCfgFile = new File(Config_file_path);

		inLogObj.setTraceID(sessionID);
		inLogObj.setClazz("com.WSM.controller.svfEncryptECDH");
		apLogObj.setTraceID(sessionID);
		apLogObj.setClazz("com.WSM.controller.svfEncryptECDH");
		...
		Log4j.log.debug(
				"[" + sessionID + "][Version: " + svVerNo + "][svfEncryptECDH] API key in header: " + headerApiKey);
		apLogObj.setMessage(
				"[" + sessionID + "][Version: " + svVerNo + "][svfEncryptECDH] API key in header: " + headerApiKey);
		...
		if (headerApiKey == null || !WSM_API_Key.equals(headerApiKey)) {
			Log4j.log.debug("[" + sessionID + "][Version: " + svVerNo
					+ "][svfEncryptECDH] Reject client request for invalid API key");
		...
		Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][svfEncryptECDH] Msg from body: " + jsonString);

		apLogObj.setMessage(
				"[" + sessionID + "][Version: " + svVerNo + "][svfEncryptECDH] Msg from body: " + jsonString);
		...
		try {
			...
			map.put("plainData", jSONObject.getString("plainData"));//TODO plainData的regexp

		} catch (JsonSyntaxException | JSONException e) {
			Log4j.log.error("[" + sessionID + "][Version: " + svVerNo
					+ "][svfEncryptECDH] Unable to parse client JSON msg: " + e.getMessage() + ", stacktrace: "
					+ new Gson().toJson(e.getStackTrace()) + ", Response: " + gts.common(ReturnCode.JsonParseError));

			apLogObj.setMessage(
					"[" + sessionID + "][Version: " + svVerNo + "][svfEncryptECDH] Error occurred while reading body: "
		...
		// log locale
		Log4j.log.trace("[" + sessionID + "][Version: " + svVerNo + "][svfEncryptECDH] Locale: " + req.getLocale());

		apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo + "][svfEncryptECDH] Locale: " + req.getLocale());
		...
		if (!map.containsKey("Error")) {
			...
			plainData = map.get("plainData");
		} else {
			Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][svfEncryptECDH] Error: " + map.get("Error")
		...
		try {
			...
			Log4j.log.error("[" + sessionID + "][Version: " + svVerNo + "][svfEncryptECDH] DB Error occurred: "
					+ e.getMessage() + ", stacktrace: " + new Gson().toJson(e.getStackTrace()));

			apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo + "][svfEncryptECDH] DB Error occurred: "
					+ e.getMessage());
			apLogObj.setThrowable(e.getMessage());
		...
		Log4j.log.trace("[" + sessionID + "][Version: " + svVerNo + "][svfEncryptECDH] Checking channel");

		if (chVO == null || "N".equals(chVO.getACTIVATE())) {
			Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][svfEncryptECDH] Rssponse: "
					+ gts.common(ReturnCode.ChannelInvalidError));

			apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo + "][svfEncryptECDH] Rssponse: "
					+ gts.common(ReturnCode.ChannelInvalidError));
			Log4jAP.log.info(apLogObj.getCompleteTxt());
		...
		Log4j.log.trace(
				"[" + sessionID + "][Version: " + svVerNo + "][svfEncryptECDH] Sending to FIDO svfEncryptECDH API");
		apLogObj.setMessage(
				"[" + sessionID + "][Version: " + svVerNo + "][svfEncryptECDH] Sending to FIDO svfEncryptECDH API");
		Log4jAP.log.info(apLogObj.getCompleteTxt());
		...
		formparams.put("method", "svfEncryptECDH");
		formparams.put("channel", channel);
		formparams.put("idgateID", idgateID);
		formparams.put("plainData", plainData);
		...
		try {
			...
			Log4j.log.error("[" + sessionID + "][Version: " + svVerNo + "][svfEncryptECDH] Connection to FIDO error: "
					+ e.getMessage() + ", stacktrace: " + new Gson().toJson(e.getStackTrace()));
		}

		Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][svfEncryptECDH] FIDO SV Response: " + retMsg);

		apLogObj.setMessage(
				"[" + sessionID + "][Version: " + svVerNo + "][svfEncryptECDH] FIDO SV Response: " + retMsg);
		Log4jAP.log.info(apLogObj.getCompleteTxt());
		...
		if (map.containsKey("Error")) {
			Log4j.log.debug("[" + sessionID + "][Version: " + svVerNo + "][svfEncryptECDH] Error: " + map.get("Error"));
		...
		try {
			...
			Log4j.log.error(
					"[" + sessionID + "][Version: " + svVerNo + "][svfEncryptECDH] Unable to parse FIDO JSON msg: "
							+ e.getMessage() + ", stacktrace: " + new Gson().toJson(e.getStackTrace()));

			apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo
					+ "][svfEncryptECDH] Unable to parse FIDO JSON msg: " + e.getMessage());
			apLogObj.setThrowable(e.getMessage());
			Log4jAP.log.error(apLogObj.getCompleteTxt());
		...
		Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][svfEncryptECDH] Response: " + retMsg);
