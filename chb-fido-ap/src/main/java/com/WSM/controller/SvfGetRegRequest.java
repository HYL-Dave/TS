...
@RestController
// @WebServlet(description = "請求裝置註冊", urlPatterns = { "/svfGetRegRequest" })
public class SvfGetRegRequest extends BaseServlet {

	@Autowired
	WSMServlet wsmServlet;

	@PostMapping("/svfGetRegRequest")
	protected String doPost(@RequestBody Object reqBody, HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		inLogObj.setClazz("com.WSM.controller.SvfGetRegRequest");
		apLogObj.setTraceID(sessionID);
		apLogObj.setClazz("com.WSM.controller.SvfGetRegRequest");
...
//				+ ", Err_msg_path: " + Err_msg_path);
		...
		Log4j.log.debug(
				"[" + sessionID + "][Version: " + svVerNo + "][SvfGetRegRequest] API key in header: " + headerApiKey);
		apLogObj.setMessage(
				"[" + sessionID + "][Version: " + svVerNo + "][SvfGetRegRequest] API key in header: " + headerApiKey);
		...
		if (headerApiKey == null || !WSM_API_Key.equals(headerApiKey)) {
			Log4j.log.debug("[" + sessionID + "][Version: " + svVerNo
					+ "][SvfGetRegRequest] Reject client request for invalid API key");
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.APIKeyInvalid));
		...
		try {
			...
			Log4j.log.fatal("[" + sessionID + "][Version: " + svVerNo
					+ "][SvfGetRegRequest] Error occurred while reading body: " + e.getMessage());
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.InternalError));
		...
		Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][SvfGetRegRequest] Msg from body: " + jsonString);

		apLogObj.setMessage(
				"[" + sessionID + "][Version: " + svVerNo + "][SvfGetRegRequest] Msg from body: " + jsonString);
		...
		try {
			...
			Log4j.log.error("[" + sessionID + "][Version: " + svVerNo
					+ "][SvfGetRegRequest] Unable to parse client JSON msg: " + e.getMessage() + ", stacktrace: "
					+ new Gson().toJson(e.getStackTrace()) + ", Response: " + gts.common(ReturnCode.JsonParseError));

			apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo
					+ "][SvfGetRegRequest] Error occurred while reading body: " + e.getMessage() + ", Response: "
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.JsonParseError));
		...
		// log locale
		Log4j.log.trace("[" + sessionID + "][Version: " + svVerNo + "][SvfGetRegRequest] Locale: " + req.getLocale());

		apLogObj.setMessage(
				"[" + sessionID + "][Version: " + svVerNo + "][SvfGetRegRequest] Locale: " + req.getLocale());
		...
		if (!map.containsKey("Error")) {
			...
			Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][SvfGetRegRequest] channel: " + channel);

			apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo + "][SvfGetRegRequest] channel: " + channel);
			Log4jAP.log.info(apLogObj.getCompleteTxt());
		} else {
			Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][SvfGetRegRequest] Error: " + map.get("Error")
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.ParameterError));
		...
		try {
			...
			Log4j.log.error("[" + sessionID + "][Version: " + svVerNo + "][SvfGetRegRequest] DB Error occurred: "
					+ e.getMessage() + ", stacktrace: " + new Gson().toJson(e.getStackTrace()));

			apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo + "][SvfGetRegRequest] DB Error occurred: "
					+ e.getMessage());
			apLogObj.setThrowable(e.getMessage());
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.DatabaseError));
		...
		Log4j.log.trace("[" + sessionID + "][Version: " + svVerNo + "][SvfGetRegRequest] Checking channel");

		if (chVO == null || "N".equals(chVO.getACTIVATE())) {
			Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][SvfGetRegRequest] Response: "
					+ gts.common(ReturnCode.ChannelInvalidError));

			apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo + "][SvfGetRegRequest] Response: "
					+ gts.common(ReturnCode.ChannelInvalidError));
			Log4jAP.log.info(apLogObj.getCompleteTxt());
...
//					.OutResult(gts.common(ReturnCode.ChannelInvalidError));
		...
		Log4j.log.trace(
				"[" + sessionID + "][Version: " + svVerNo + "][SvfGetRegRequest] Sending to FIDO SvfGetRegRequest API");
		apLogObj.setMessage(
				"[" + sessionID + "][Version: " + svVerNo + "][SvfGetRegRequest] Sending to FIDO SvfGetRegRequest API");
		Log4jAP.log.info(apLogObj.getCompleteTxt());
		...
		formparams.put("method", "svfGetRegRequest");
...
//			retMsg = new Send2Remote().sendPost(FIDO_SV_IP, WSM_API_Key, gson.toJson(formparams));
			...
			Log4j.log.error("[" + sessionID + "][Version: " + svVerNo + "][SvfGetRegRequest] Connection to FIDO error: "
					+ e.getMessage() + ", stacktrace: " + new Gson().toJson(e.getStackTrace()));

			apLogObj.setMessage(
					"[" + sessionID + "][Version: " + svVerNo + "][SvfGetRegRequest] Connection to FIDO error: "
							+ e.getMessage() + ", stacktrace: " + new Gson().toJson(e.getStackTrace()));
			apLogObj.setThrowable(e.getMessage());
			Log4jAP.log.error(apLogObj.getCompleteTxt());
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.FidoSVError));
		...
		Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][SvfGetRegRequest] FIDO SV Response: " + retMsg);

		apLogObj.setMessage(
				"[" + sessionID + "][Version: " + svVerNo + "][SvfGetRegRequest] FIDO SV Response: " + retMsg);
		Log4jAP.log.info(apLogObj.getCompleteTxt());
		...
		if (map.containsKey("Error")) {
			Log4j.log.debug(
					"[" + sessionID + "][Version: " + svVerNo + "][SvfGetRegRequest] Error: " + map.get("Error"));
		...
		try {
			...
			Log4j.log.error(
					"[" + sessionID + "][Version: " + svVerNo + "][SvfGetRegRequest] Unable to parse FIDO JSON msg: "
							+ e.getMessage() + ", stacktrace: " + new Gson().toJson(e.getStackTrace()));

			apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo
					+ "][SvfGetRegRequest] Unable to parse FIDO JSON msg: " + e.getMessage());
			apLogObj.setThrowable(e.getMessage());
			Log4jAP.log.error(apLogObj.getCompleteTxt());
		...
		Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][SvfGetRegRequest] Response: " + retMsg);
