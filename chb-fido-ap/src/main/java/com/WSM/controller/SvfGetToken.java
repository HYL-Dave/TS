...
/**
 * Servlet implementation class SvfGetToken
 */
@RestController
// @WebServlet(description = "取得Token", urlPatterns = { "/svfGetToken" })
public class SvfGetToken extends BaseServlet {

	@Autowired
	WSMServlet wsmServlet;

	@PostMapping("/svfGetToken")
	protected String doPost(@RequestBody Object reqBody, HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		inLogObj.setClazz("com.WSM.controller.SvfGetToken");
		apLogObj.setTraceID(sessionID);
		apLogObj.setClazz("com.WSM.controller.SvfGetToken");
...
//				+ ", Err_msg_path: " + Err_msg_path);
		...
		Log4j.log
				.debug("[" + sessionID + "][Version: " + svVerNo + "][SvfGetToken] API key in header: " + headerApiKey);
		apLogObj.setMessage(
				"[" + sessionID + "][Version: " + svVerNo + "][SvfGetToken] API key in header: " + headerApiKey);
		...
		if (headerApiKey == null || !WSM_API_Key.equals(headerApiKey)) {
			Log4j.log.debug("[" + sessionID + "][Version: " + svVerNo
					+ "][SvfGetToken] Reject client request for invalid API key");
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.APIKeyInvalid));
		...
		try {
			...
			Log4j.log.fatal("[" + sessionID + "][Version: " + svVerNo
					+ "][SvfGetToken] Error occurred while reading body: " + e.getMessage());
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.InternalError));
		...
		Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][SvfGetToken] Msg from body: " + jsonString);

		apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo + "][SvfGetToken] Msg from body: " + jsonString);
		...
		try {
			...
			Log4j.log.error("[" + sessionID + "][Version: " + svVerNo
					+ "][SvfGetToken] Unable to parse client JSON msg: " + e.getMessage() + ", stacktrace: "
					+ new Gson().toJson(e.getStackTrace()) + ", Response: " + gts.common(ReturnCode.JsonParseError));

			apLogObj.setMessage(
					"[" + sessionID + "][Version: " + svVerNo + "][SvfGetToken] Error occurred while reading body: "
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.JsonParseError));
		...
		// log locale
		Log4j.log.trace("[" + sessionID + "][Version: " + svVerNo + "][SvfGetToken] Locale: " + req.getLocale());

		apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo + "][SvfGetToken] Locale: " + req.getLocale());
		...
		if (!map.containsKey("Error")) {
			...
			Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][SvfGetToken] channel: " + channel
					+ ", idgateID: " + idgateID);

			apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo + "][SvfGetToken] channel: " + channel
					+ ", idgateID: " + idgateID);
			Log4jAP.log.info(apLogObj.getCompleteTxt());
		} else {
			Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][SvfGetToken] Error: " + map.get("Error")
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.ParameterError));
		...
		try {
			...
			Log4j.log.error("[" + sessionID + "][Version: " + svVerNo + "][SvfGetToken] DB Error occurred: "
					+ e.getMessage() + ", stacktrace: " + new Gson().toJson(e.getStackTrace()));

			apLogObj.setMessage(
					"[" + sessionID + "][Version: " + svVerNo + "][SvfGetToken] DB Error occurred: " + e.getMessage());
			apLogObj.setThrowable(e.getMessage());
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.DatabaseError));
		...
		if (chVO == null || "N".equals(chVO.getACTIVATE())) {
			Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][SvfGetToken] Response: "
					+ gts.common(ReturnCode.ChannelInvalidError));

			apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo + "][SvfGetToken] Response: "
					+ gts.common(ReturnCode.ChannelInvalidError));
			Log4jAP.log.info(apLogObj.getCompleteTxt());
...
//					.OutResult(gts.common(ReturnCode.ChannelInvalidError));
		...
		Log4j.log.trace("[" + sessionID + "][Version: " + svVerNo + "][svfGetToken] Sending to FIDO svfGetToken API");
		apLogObj.setMessage(
				"[" + sessionID + "][Version: " + svVerNo + "][svfGetToken] Sending to FIDO svfGetToken API");
		Log4jAP.log.info(apLogObj.getCompleteTxt());

		// outward data to FIDO server
		HashMap<String, String> formparams = new HashMap<String, String>();
		formparams.put("method", "svfGetToken");
...
//			retMsg = new Send2Remote().sendPost(FIDO_SV_IP, WSM_API_Key, gson.toJson(formparams));
			...
			Log4j.log.error("[" + sessionID + "][Version: " + svVerNo + "][SvfGetToken] Connection to FIDO error: "
					+ e.getMessage() + ", stacktrace: " + new Gson().toJson(e.getStackTrace()));

			apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo + "][SvfGetToken] Connection to FIDO error: "
					+ e.getMessage() + ", stacktrace: " + new Gson().toJson(e.getStackTrace()));
			apLogObj.setThrowable(e.getMessage());
			Log4jAP.log.error(apLogObj.getCompleteTxt());
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.FidoSVError));
		...
		Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][SvfGetToken] FIDO SV Response: " + retMsg);

		apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo + "][SvfGetToken] FIDO SV Response: " + retMsg);
		Log4jAP.log.info(apLogObj.getCompleteTxt());
		...
		if (map.containsKey("Error")) {
			Log4j.log.debug("[" + sessionID + "][Version: " + svVerNo + "][SvfGetToken] Error: " + map.get("Error"));
		...
		try {
			...
			Log4j.log.error(
					"[" + sessionID + "][Version: " + svVerNo + "][SvfGetToken] Unable to parse FIDO JSON msg: "
							+ e.getMessage() + ", stacktrace: " + new Gson().toJson(e.getStackTrace()));

			apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo
					+ "][SvfGetToken] Unable to parse FIDO JSON msg: " + e.getMessage());
			apLogObj.setThrowable(e.getMessage());
			Log4jAP.log.error(apLogObj.getCompleteTxt());
		...
		Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][SvfGetToken] Response: " + retMsg);
