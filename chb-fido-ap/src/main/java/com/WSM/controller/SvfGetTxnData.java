...
/**
 * Servlet implementation class SvfGetTxnData
 */
@RestController
// @WebServlet(description = "取得加密交易資料", urlPatterns = { "/svfGetTxnData" })
public class SvfGetTxnData extends BaseServlet {

	@Autowired
	WSMServlet wsmServlet;

	@PostMapping("/svfGetTxnData")
	protected String doPost(@RequestBody Object reqBody, HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		inLogObj.setClazz("com.WSM.controller.SvfGetTxnData");
		apLogObj.setTraceID(sessionID);
		apLogObj.setClazz("com.WSM.controller.SvfGetTxnData");
...
//				+ ", Err_msg_path: " + Err_msg_path);
		...
		Log4j.log.debug(
				"[" + sessionID + "][Version: " + svVerNo + "][SvfGetTxnData] API key in header: " + headerApiKey);
		apLogObj.setMessage(
				"[" + sessionID + "][Version: " + svVerNo + "][SvfGetTxnData] API key in header: " + headerApiKey);
		...
		if (headerApiKey == null || !WSM_API_Key.equals(headerApiKey)) {
			Log4j.log.debug("[" + sessionID + "][Version: " + svVerNo
					+ "][SvfGetTxnData] Reject client request for invalid API key");
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.APIKeyInvalid));
		...
		try {
			...
			Log4j.log.fatal("[" + sessionID + "][Version: " + svVerNo
					+ "][SvfGetTxnData] Error occurred while reading body: " + e.getMessage());
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.InternalError));
		...
		Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][SvfGetTxnData] Msg from body: " + jsonString);

		apLogObj.setMessage(
				"[" + sessionID + "][Version: " + svVerNo + "][SvfGetTxnData] Msg from body: " + jsonString);
		...
		try {
			...
			map.put("encTxnID", jSONObject.getString("encTxnID"));

		} catch (JsonSyntaxException | JSONException e) {
			Log4j.log.error("[" + sessionID + "][Version: " + svVerNo
					+ "][SvfGetTxnData] Unable to parse client JSON msg: " + e.getMessage() + ", stacktrace: "
					+ new Gson().toJson(e.getStackTrace()) + ", Response: " + gts.common(ReturnCode.JsonParseError));

			apLogObj.setMessage(
					"[" + sessionID + "][Version: " + svVerNo + "][SvfGetTxnData] Error occurred while reading body: "
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.JsonParseError));
		...
		// log locale
		Log4j.log.trace("[" + sessionID + "][Version: " + svVerNo + "][SvfGetTxnData] Locale: " + req.getLocale());

		apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo + "][SvfGetTxnData] Locale: " + req.getLocale());
		...
		if (!map.containsKey("Error")) {
			...
			encTxnID = map.get("encTxnID");

			Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][SvfGetTxnData] channel: " + channel
					+ ", idgateID: " + idgateID + ", encTxnID: " + encTxnID);

			apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo + "][SvfGetTxnData] channel: " + channel
					+ ", idgateID: " + idgateID + ", encTxnID: " + encTxnID);
			Log4jAP.log.info(apLogObj.getCompleteTxt());
		} else {
			Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][SvfGetTxnData] Error: " + map.get("Error")
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.ParameterError));
		...
		try {
			...
			Log4j.log.error("[" + sessionID + "][Version: " + svVerNo + "][SvfGetTxnData] DB Error occurred: "
					+ e.getMessage() + ", stacktrace: " + new Gson().toJson(e.getStackTrace()));

			apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo + "][SvfGetTxnData] DB Error occurred: "
					+ e.getMessage());
			apLogObj.setThrowable(e.getMessage());
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.DatabaseError));
		...
		if (chVO == null || "N".equals(chVO.getACTIVATE())) {
			Log4j.log.info("[" + sessionID + "][Version: " + svVerNo
					+ "][SvfGetTxnData] Channel doesn't exist or disabled. Response: "
...
//					.OutResult(gts.common(ReturnCode.ChannelInvalidError));
		...
		Log4j.log.trace(
				"[" + sessionID + "][Version: " + svVerNo + "][svfGetTxnData] Sending to FIDO svfGetTxnData API");
		apLogObj.setMessage(
				"[" + sessionID + "][Version: " + svVerNo + "][svfGetTxnData] Sending to FIDO svfGetTxnData API");
		Log4jAP.log.info(apLogObj.getCompleteTxt());
		...
		formparams.put("method", "svfGetTxnData");
		formparams.put("channel", channel);
		formparams.put("idgateID", idgateID);
		formparams.put("encTxnID", encTxnID);
...
//			retMsg = new Send2Remote().sendPost(FIDO_SV_IP, WSM_API_Key, gson.toJson(formparams));
			...
			Log4j.log.error("[" + sessionID + "][Version: " + svVerNo + "][SvfGetTxnData] Connection to FIDO error: "
					+ e.getMessage() + ", stacktrace: " + new Gson().toJson(e.getStackTrace()));

			apLogObj.setMessage(
					"[" + sessionID + "][Version: " + svVerNo + "][SvfGetTxnData] Connection to FIDO error: "
							+ e.getMessage() + ", stacktrace: " + new Gson().toJson(e.getStackTrace()));
			apLogObj.setThrowable(e.getMessage());
			Log4jAP.log.error(apLogObj.getCompleteTxt());
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.FidoSVError));
		...
		Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][SvfGetTxnData] FIDO SV Response: " + retMsg);

		apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo + "][SvfGetTxnData] FIDO SV Response: " + retMsg);
		Log4jAP.log.info(apLogObj.getCompleteTxt());
		...
		if (map.containsKey("Error")) {
			Log4j.log.debug("[" + sessionID + "][Version: " + svVerNo + "][SvfGetTxnData] Error: " + map.get("Error"));
		...
		try {
			...
			Log4j.log.error(
					"[" + sessionID + "][Version: " + svVerNo + "][SvfGetTxnData] Unable to parse FIDO JSON msg: "
							+ e.getMessage() + ", stacktrace: " + new Gson().toJson(e.getStackTrace()));

			apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo
					+ "][SvfGetTxnData] Unable to parse FIDO JSON msg: " + e.getMessage());
			apLogObj.setThrowable(e.getMessage());
			Log4jAP.log.error(apLogObj.getCompleteTxt());
		...
		Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][SvfGetTxnData] Response: " + retMsg);
