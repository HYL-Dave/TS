...
/**
 * Servlet implementation class SvfOfflineOTPRequest
 */
@RestController
// @WebServlet(description = "請求離線式OTP驗證", urlPatterns = { "/svfOfflineOTPRequest" })
public class SvfOfflineOTPRequest extends BaseServlet {

	@Autowired
	WSMServlet wsmServlet;
	
	@PostMapping("/svfOfflineOTPRequest")
	protected String doPost(@RequestBody Object reqBody, HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		inLogObj.setClazz("com.WSM.controller.SvfOfflineOTPRequest");
		apLogObj.setTraceID(sessionID);
		apLogObj.setClazz("com.WSM.controller.SvfOfflineOTPRequest");
...
//				+ ", Err_msg_path: " + Err_msg_path);
		...
		Log4j.log.debug("[" + sessionID + "][Version: " + svVerNo + "][SvfOfflineOTPRequest] API key in header: "
				+ headerApiKey);
		apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo + "][SvfOfflineOTPRequest] API key in header: "
				+ headerApiKey);
		...
		if (headerApiKey == null || !WSM_API_Key.equals(headerApiKey)) {
			Log4j.log.debug("[" + sessionID + "][Version: " + svVerNo
					+ "][SvfOfflineOTPRequest] Reject client request for invalid API key");
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.APIKeyInvalid));
		...
		try {
			...
			Log4j.log.fatal("[" + sessionID + "][Version: " + svVerNo
					+ "][SvfOfflineOTPRequest] Error occurred while reading body: " + e.getMessage());
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.InternalError));
		...
		Log4j.log.info(
				"[" + sessionID + "][Version: " + svVerNo + "][SvfOfflineOTPRequest] Msg from body: " + jsonString);
		...
		try {
			...
			map.put("txnTitle", jSONObject.getString("txnTitle"));

		} catch (JsonSyntaxException | JSONException e) {
			Log4j.log.error("[" + sessionID + "][Version: " + svVerNo
					+ "][SvfOfflineOTPRequest] Unable to parse client JSON msg: " + e.getMessage() + ", stacktrace: "
					+ new Gson().toJson(e.getStackTrace()) + ", Response: " + gts.common(ReturnCode.JsonParseError));

			apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo
					+ "][SvfOfflineOTPRequest] Error occurred while reading body: " + e.getMessage() + ", Response: "
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.JsonParseError));
		...
		// log locale
		Log4j.log.trace(
				"[" + sessionID + "][Version: " + svVerNo + "][SvfOfflineOTPRequest] Locale: " + req.getLocale());
		...
		if (!map.containsKey("Error")) {
			...
			txnTitle = map.get("txnTitle");

			Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][SvfOfflineOTPRequest] channel: " + channel
					+ ", idgateID: " + idgateID + ", txnTitle: " + txnTitle + ", bankTxnData: " + bankTxnData);

			apLogObj.setMessage(
					"[" + sessionID + "][Version: " + svVerNo + "][SvfOfflineOTPRequest] channel: " + channel
							+ ", idgateID: " + idgateID + ", txnTitle: " + txnTitle + ", bankTxnData: " + bankTxnData);
			Log4jAP.log.info(apLogObj.getCompleteTxt());
		} else {
			Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][SvfOfflineOTPRequest] Error: "
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.ParameterError));
		...
		try {
			...
			Log4j.log.error("[" + sessionID + "][Version: " + svVerNo + "][SvfOfflineOTPRequest] DB Error occurred: "
					+ e.getMessage() + ", stacktrace: " + new Gson().toJson(e.getStackTrace()));

			apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo
					+ "][SvfOfflineOTPRequest] DB Error occurred: " + e.getMessage());
			apLogObj.setThrowable(e.getMessage());
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.DatabaseError));
		...
		if (chVO == null || "N".equals(chVO.getACTIVATE())) {
			Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][SvfOfflineOTPRequest] Response: "
					+ gts.common(ReturnCode.ChannelInvalidError));

			apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo + "][SvfOfflineOTPRequest] Response: "
					+ gts.common(ReturnCode.ChannelInvalidError));
			Log4jAP.log.info(apLogObj.getCompleteTxt());
...
//					.OutResult(gts.common(ReturnCode.ChannelInvalidError));
		...
		// check if it's not an active account
		if (!memVO.getCustomer_Status().equals(MemberStatus.Normal)) {
			Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][SvfOfflineOTPRequest] Response: "
...
//					.OutResult(gts.common(ReturnCode.MemberStatusError));
		...
		} else if (!memVO.getVerify_Type().equals(VerifyType.OFFLINE)) {
			Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][SvfOfflineOTPRequest] Response: "
					+ gts.common(ReturnCode.CannotUseQRCodeInOfflineOTPRequest));
			...
			inLogObj.setExecuteTime(System.currentTimeMillis() - startTime);
			inLogObj.setResponseTxt(gts.common(ReturnCode.CannotUseQRCodeInOfflineOTPRequest));
			Log4jInbound.log.info(inLogObj.getCompleteTxt());

//			new OutputHandler(res, "application/json; charset=UTF-8")
//					.OutResult(gts.common(ReturnCode.CannotUseQRCodeInOfflineOTPRequest));
			return gts.common(ReturnCode.CannotUseQRCodeInOfflineOTPRequest);
		}

		Log4j.log.trace("[" + sessionID + "][Version: " + svVerNo
				+ "][svfOfflineOTPRequest] Sending to FIDO svfOfflineOTPRequest API");
		apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo
				+ "][svfOfflineOTPRequest] Sending to FIDO svfOfflineOTPRequest API");
		Log4jAP.log.info(apLogObj.getCompleteTxt());
		...
		formparams.put("method", "svfOfflineOTPRequest");
		formparams.put("channel", channel);
		formparams.put("idgateID", idgateID);
		formparams.put("bankTxnData", bankTxnData);
		formparams.put("txnTitle", txnTitle);
...
//			retMsg = new Send2Remote().sendPost(FIDO_SV_IP, WSM_API_Key, gson.toJson(formparams));
			...
			Log4j.log.error(
					"[" + sessionID + "][Version: " + svVerNo + "][SvfOfflineOTPRequest] Connection to FIDO error: "
							+ e.getMessage() + ", stacktrace: " + new Gson().toJson(e.getStackTrace()));
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.FidoSVError));
		...
		Log4j.log.info(
				"[" + sessionID + "][Version: " + svVerNo + "][SvfOfflineOTPRequest] FIDO SV Response: " + retMsg);
		...
		if (map.containsKey("Error")) {
			Log4j.log.debug(
					"[" + sessionID + "][Version: " + svVerNo + "][SvfOfflineOTPRequest] Error: " + map.get("Error"));
		...
		try {
			...
			Log4j.log.error(
					"[" + sessionID + "][Version: " + svVerNo + "][SvfOfflineOTPRequest] Unable to parse FIDO JSON msg: "
							+ e.getMessage() + ", stacktrace: " + new Gson().toJson(e.getStackTrace()));

			apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo
					+ "][SvfOfflineOTPRequest] Unable to parse FIDO JSON msg: " + e.getMessage());
			apLogObj.setThrowable(e.getMessage());
			Log4jAP.log.error(apLogObj.getCompleteTxt());
		...
		Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][SvfOfflineOTPRequest] Response: " + retMsg);

		inLogObj.setExecuteTime(System.currentTimeMillis() - startTime);
		inLogObj.setResponseTxt(retMsg);
		Log4jInbound.log.info(inLogObj.getCompleteTxt());

//		new OutputHandler(res, "application/json; charset=UTF-8").OutResult(retMsg.replace("\\", ""));
		return retMsg;
	}
}
