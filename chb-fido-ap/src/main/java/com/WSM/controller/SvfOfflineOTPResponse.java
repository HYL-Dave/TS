...
/**
 * Servlet implementation class SvfOfflineOTPResponse
 */
@RestController
// @WebServlet(description = "回覆離線式OTP驗證", urlPatterns = { "/svfOfflineOTPResponse" })
public class SvfOfflineOTPResponse extends BaseServlet {
	...
	@PostMapping("/svfOfflineOTPResponse")
	protected String doPost(@RequestBody Object reqBody, HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		inLogObj.setClazz("com.WSM.controller.SvfOfflineOTPResponse");
		apLogObj.setTraceID(sessionID);
		apLogObj.setClazz("com.WSM.controller.SvfOfflineOTPResponse");
...
//				+ ", Err_msg_path: " + Err_msg_path);
		...
		Log4j.log.debug("[" + sessionID + "][Version: " + svVerNo + "][SvfOfflineOTPResponse] API key in header: "
				+ headerApiKey);
		apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo + "][SvfOfflineOTPResponse] API key in header: "
				+ headerApiKey);
		...
		if (headerApiKey == null || !WSM_API_Key.equals(headerApiKey)) {
			Log4j.log.debug("[" + sessionID + "][Version: " + svVerNo
					+ "][SvfOfflineOTPResponse] Reject client request for invalid API key");
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.APIKeyInvalid));
		...
		try {
			...
			Log4j.log.fatal("[" + sessionID + "][Version: " + svVerNo
					+ "][SvfOfflineOTPResponse] Error occurred while reading body: " + e.getMessage());
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.InternalError));
		...
		Log4j.log.info(
				"[" + sessionID + "][Version: " + svVerNo + "][SvfOfflineOTPResponse] Msg from body: " + jsonString);
		...
		try {
			...
			map.put("otp", jSONObject.getString("otp"));

		} catch (JsonSyntaxException | JSONException e) {
			Log4j.log.error("[" + sessionID + "][Version: " + svVerNo
					+ "][SvfOfflineOTPResponse] Unable to parse client JSON msg: " + e.getMessage() + ", stacktrace: "
					+ new Gson().toJson(e.getStackTrace()) + ", Response: " + gts.common(ReturnCode.JsonParseError));

			apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo
					+ "][SvfOfflineOTPResponse] Error occurred while reading body: " + e.getMessage() + ", Response: "
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.JsonParseError));
		...
		// log locale
		Log4j.log.trace(
				"[" + sessionID + "][Version: " + svVerNo + "][SvfOfflineOTPResponse] Locale: " + req.getLocale());
		...
		if (!map.containsKey("Error")) {
			...
			Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][SvfOfflineOTPResponse] channel: " + channel
					+ ", idgateID: " + idgateID + ", txnID: " + txnID + ", otp: " + otp);

			apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo + "][SvfOfflineOTPResponse] channel: "
					+ channel + ", idgateID: " + idgateID + ", txnID: " + txnID + ", otp: " + otp);
			Log4jAP.log.info(apLogObj.getCompleteTxt());
		} else {
			Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][SvfOfflineOTPResponse] Error: "
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.ParameterError));
		...
		try {
			...
			Log4j.log.error("[" + sessionID + "][Version: " + svVerNo + "][SvfOfflineOTPResponse] DB Error occurred: "
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.DatabaseError));
		...
		if (chVO == null || "N".equals(chVO.getACTIVATE())) {
			Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][SvfOfflineOTPResponse] Response: "
					+ gts.common(ReturnCode.ChannelInvalidError));

			apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo + "][SvfOfflineOTPResponse] Response: "
					+ gts.common(ReturnCode.ChannelInvalidError));
			Log4jAP.log.info(apLogObj.getCompleteTxt());
...
//					.OutResult(gts.common(ReturnCode.MemberStatusError));
		...
		Log4j.log.trace("[" + sessionID + "][Version: " + svVerNo
				+ "][svfOfflineOTPResponse] Sending to FIDO svfOfflineOTPResponse API");
		apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo
				+ "][svfOfflineOTPResponse] Sending to FIDO svfOfflineOTPResponse API");
		Log4jAP.log.info(apLogObj.getCompleteTxt());
...
//			retMsg = new Send2Remote().sendPost(FIDO_SV_IP, WSM_API_Key, gson.toJson(formparams));
			...
			if (dataSet.containsKey("failCount") && !dataSet.get("failCount").equals("0")) {
				...
				Log4j.log.debug("[" + sessionID + "][Version: " + svVerNo
						+ "][svfOfflineOTPResponse] failCount received from FIDO SV: " + failCount);
			}
			...
			if (failCount > 0) {
				...
				Log4j.log.debug("[" + sessionID + "][Version: " + svVerNo
						+ "][svfOfflineOTPResponse] FIDO device status response: " + devStatus);

				JSONObject devJson = new JSONObject(devStatus);

				if (memVO.getVerify_Type().equals(VerifyType.OFFLINE)) {
					// offline verification failed
					memSvc.updateOfflineAuthFails(idgID, failCount);
					...
					memSvc.updateLoginAuthFails(idgID, failCount);
					...
					memSvc.updateTxnAuthFails(idgID, failCount);
			...
			Log4j.log.error(
					"[" + sessionID + "][Version: " + svVerNo + "][SvfOfflineOTPResponse] Connection to FIDO error: "
							+ e.getMessage() + ", stacktrace: " + new Gson().toJson(e.getStackTrace()));
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.DatabaseError));
		...
		Log4j.log.info(
				"[" + sessionID + "][Version: " + svVerNo + "][SvfOfflineOTPResponse] FIDO SV Response: " + retMsg);
		...
		if (map.containsKey("Error")) {
			Log4j.log.debug(
					"[" + sessionID + "][Version: " + svVerNo + "][SvfOfflineOTPResponse] Error: " + map.get("Error"));
		...
		try {
			...
			Log4j.log.error("[" + sessionID + "][Version: " + svVerNo
					+ "][SvfOfflineOTPResponse] Unable to parse FIDO JSON msg: " + e.getMessage() + ", stacktrace: "
					+ new Gson().toJson(e.getStackTrace()));

			apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo
					+ "][SvfOfflineOTPResponse] Unable to parse FIDO JSON msg: " + e.getMessage());
			apLogObj.setThrowable(e.getMessage());
			Log4jAP.log.error(apLogObj.getCompleteTxt());
		...
		Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][SvfOfflineOTPResponse] Response: " + retMsg);
