...
/**
 * Servlet implementation class SvfSendAuthResponse
 */
@RestController
// @WebServlet(description = "確認驗證結果", urlPatterns = { "/svfSendAuthResponse" })
public class SvfSendAuthResponse extends BaseServlet {
	...
	@PostMapping("/svfSendAuthResponse")
	protected String doPost(@RequestBody Object reqBody, HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		inLogObj.setClazz("com.WSM.controller.SvfSendAuthResponse");
		apLogObj.setTraceID(sessionID);
		apLogObj.setClazz("com.WSM.controller.SvfSendAuthResponse");
...
//				+ ", Err_msg_path: " + Err_msg_path);
		...
		Log4j.log.debug("[" + sessionID + "][Version: " + svVerNo + "][SvfSendAuthResponse] API key in header: "
				+ headerApiKey);
		apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo + "][SvfSendAuthResponse] API key in header: "
				+ headerApiKey);
		...
		if (headerApiKey == null || !WSM_API_Key.equals(headerApiKey)) {
			Log4j.log.debug("[" + sessionID + "][Version: " + svVerNo
					+ "][SvfSendAuthResponse] Reject client request for invalid API key");
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.APIKeyInvalid));
		...
		try {
			...
			Log4j.log.fatal("[" + sessionID + "][Version: " + svVerNo
					+ "][SvfSendAuthResponse] Error occurred while reading body: " + e.getMessage());
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.InternalError));
		...
		Log4j.log.info(
				"[" + sessionID + "][Version: " + svVerNo + "][SvfSendAuthResponse] Msg from body: " + jsonString);
		...
		try {
			...
			map.put("bankTxnID", jSONObject.getString("bankTxnID"));
			map.put("idgateID", jSONObject.getString("idgateID"));
			map.put("encVerifyTxnData", jSONObject.getString("encVerifyTxnData"));

		} catch (JsonSyntaxException | JSONException e) {
			Log4j.log.error("[" + sessionID + "][Version: " + svVerNo
					+ "][SvfSendAuthResponse] Unable to parse client JSON msg: " + e.getMessage() + ", stacktrace: "
					+ new Gson().toJson(e.getStackTrace()) + ", Response: " + gts.common(ReturnCode.JsonParseError));

			apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo
					+ "][SvfSendAuthResponse] Error occurred while reading body: " + e.getMessage() + ", Response: "
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.JsonParseError));
		...
		// log locale
		Log4j.log
				.trace("[" + sessionID + "][Version: " + svVerNo + "][SvfSendAuthResponse] Locale: " + req.getLocale());

		apLogObj.setMessage(
				"[" + sessionID + "][Version: " + svVerNo + "][SvfSendAuthResponse] Locale: " + req.getLocale());
		Log4jAP.log.trace(apLogObj.getCompleteTxt());

		String channel;
		String idgateID;
		String bankTxnID;
		String encVerifyTxnData;
		if (!map.containsKey("Error")) {
			channel = map.get("channel");
			idgateID = map.get("idgateID");
			bankTxnID = map.get("bankTxnID");
			encVerifyTxnData = map.get("encVerifyTxnData");

			Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][SvfSendAuthResponse] channel: " + channel
					+ ", idgateID: " + idgateID + ", bankTxnID: " + bankTxnID + ", encVerifyTxnData: "
					+ encVerifyTxnData);

			apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo + "][SvfSendAuthResponse] channel: " + channel
					+ ", idgateID: " + idgateID + ", bankTxnID: " + bankTxnID + ", encVerifyTxnData: "
					+ encVerifyTxnData);
			Log4jAP.log.info(apLogObj.getCompleteTxt());

		} else {
			Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][SvfSendAuthResponse] Error: "
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.ParameterError));
		...
		try {
			...
			Log4j.log.error("[" + sessionID + "][Version: " + svVerNo + "][SvfSendAuthResponse] DB Error occurred: "
					+ e.getMessage() + ", stacktrace: " + new Gson().toJson(e.getStackTrace()));

			apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo + "][SvfSendAuthResponse] DB Error occurred: "
					+ e.getMessage());
			apLogObj.setThrowable(e.getMessage());
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.DatabaseError));
		...
		if (chVO == null || "N".equals(chVO.getACTIVATE())) {
			Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][SvfSendAuthResponse] Response: "
...
//					.OutResult(gts.common(ReturnCode.ChannelInvalidError));
		...
		} else if (memVO == null || memVO.getVerify_Type().equals(VerifyType.OFFLINE)) {
			// member doesn't exist
			Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][SvfSendAuthResponse] Response: "
					+ gts.common(ReturnCode.MemberNotFound));

			apLogObj.setMessage(gts.common(ReturnCode.MemberNotFound));
			Log4jAP.log.info(apLogObj.getCompleteTxt());
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.MemberNotFound));
			...
			apLogObj.setMessage(gts.common_w_Failcount(ReturnCode.MemberLockedError, failCount));
			Log4jAP.log.info(apLogObj.getCompleteTxt());
...
//					.OutResult(gts.common_w_Failcount(ReturnCode.MemberLockedError, failCount));
			...
			apLogObj.setMessage(gts.common(ReturnCode.MemberDeletedError));
			Log4jAP.log.info(apLogObj.getCompleteTxt());
...
//					.OutResult(gts.common(ReturnCode.MemberDeletedError));
			...
			apLogObj.setMessage(gts.common(ReturnCode.MemberStatusError));
			Log4jAP.log.info(apLogObj.getCompleteTxt());
...
//					.OutResult(gts.common(ReturnCode.MemberStatusError));
		...
		Log4j.log.trace("[" + sessionID + "][Version: " + svVerNo
				+ "][svfSendAuthResponse] Sending to FIDO svfSendAuthResponse API");
		apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo
				+ "][svfSendAuthResponse] Sending to FIDO svfSendAuthResponse API");
		Log4jAP.log.info(apLogObj.getCompleteTxt());
		...
		formparams.put("method", "svfSendAuthResponse");
		formparams.put("channel", channel);
		formparams.put("idgateID", idgateID);
		formparams.put("bankTxnID", bankTxnID);
		formparams.put("encVerifyTxnData", encVerifyTxnData);

		HashMap<String, String> dataSet = null;
		String retMsg = null;
		int failCount = 0;
		try {
			// receive and process response of svfSendAuthResponse
			// DONE
...
//			retMsg = new Send2Remote().sendPost(FIDO_SV_IP, WSM_API_Key, gson.toJson(formparams));
			...
			if (dataSet.containsKey("failCount") && !dataSet.get("failCount").equals("0")) {
				...
				Log4j.log.debug("[" + sessionID + "][Version: " + svVerNo
						+ "][svfSendAuthResponse] FIDO device status response: " + devStatus);
				...
				if (memVO.getVerify_Type().equals(VerifyType.OFFLINE)) {
					...
					// quick login verification failed
			...
			Log4j.log.error(
					"[" + sessionID + "][Version: " + svVerNo + "][SvfSendAuthResponse] Connection to FIDO error: "
							+ e.getMessage() + ", stacktrace: " + new Gson().toJson(e.getStackTrace()));
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.DatabaseError));
		...
		try {
			...
			Log4j.log.error(
					"[" + sessionID + "][Version: " + svVerNo + "][SvfSendAuthResponse] Unable to parse FIDO JSON msg: "
							+ e.getMessage() + ", stacktrace: " + new Gson().toJson(e.getStackTrace()));

			apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo
					+ "][SvfSendAuthResponse] Unable to parse FIDO JSON msg: " + e.getMessage());
			apLogObj.setThrowable(e.getMessage());
			Log4jAP.log.error(apLogObj.getCompleteTxt());
		...
		Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][SvfSendAuthResponse] Response: " + retMsg);

		apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo + "][SvfSendAuthResponse] Response: " + retMsg);
		Log4jAP.log.info(apLogObj.getCompleteTxt());
