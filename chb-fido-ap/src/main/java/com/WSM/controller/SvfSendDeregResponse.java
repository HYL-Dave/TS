...
/**
 * Servlet implementation class SvfSendDeregResponse
 */
@RestController
// @WebServlet(description = "註銷裝置綁定", urlPatterns = { "/svfSendDeregResponse" })
public class SvfSendDeregResponse extends BaseServlet {

	@Autowired
	WSMServlet wsmServlet;

	@PostMapping("/svfSendDeregResponse")
	protected String doPost(@RequestBody Object reqBody, HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		inLogObj.setClazz("com.WSM.controller.SvfSendDeregResponse");
		apLogObj.setTraceID(sessionID);
		apLogObj.setClazz("com.WSM.controller.SvfSendDeregResponse");
...
//				+ ", Err_msg_path: " + Err_msg_path);
		...
		Log4j.log.debug("[" + sessionID + "][Version: " + svVerNo + "][SvfSendDeregResponse] API key in header: "
				+ headerApiKey);
		apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo + "][SvfSendDeregResponse] API key in header: "
				+ headerApiKey);
		...
		if (headerApiKey == null || !WSM_API_Key.equals(headerApiKey)) {
			Log4j.log.debug("[" + sessionID + "][Version: " + svVerNo
					+ "][SvfSendDeregResponse] Reject client request for invalid API key");
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.APIKeyInvalid));
		...
		try {
			...
			Log4j.log.fatal("[" + sessionID + "][Version: " + svVerNo
					+ "][SvfSendDeregResponse] Error occurred while reading body: " + e.getMessage());
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.InternalError));
		...
		Log4j.log.info(
				"[" + sessionID + "][Version: " + svVerNo + "][SvfSendDeregResponse] Msg from body: " + jsonString);
		...
		try {
			...
			map.put("encDeregData", jSONObject.getString("encDeregData"));

		} catch (JsonSyntaxException | JSONException e) {
			Log4j.log.error("[" + sessionID + "][Version: " + svVerNo
					+ "][SvfSendDeregResponse] Unable to parse client JSON msg: " + e.getMessage() + ", stacktrace: "
					+ new Gson().toJson(e.getStackTrace()) + ", Response: " + gts.common(ReturnCode.JsonParseError));

			apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo
					+ "][SvfSendDeregResponse] Error occurred while reading body: " + e.getMessage() + ", Response: "
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.JsonParseError));
		...
		// log locale
		Log4j.log.trace(
				"[" + sessionID + "][Version: " + svVerNo + "][SvfSendDeregResponse] Locale: " + req.getLocale());
		...
		if (!map.containsKey("Error")) {
			...
			encDeregData = map.get("encDeregData");

			Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][SvfSendDeregResponse] channel: " + channel
					+ ", idgateID: " + idgateID + ", encDeregData: " + encDeregData);

			apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo + "][SvfSendDeregResponse] channel: "
					+ channel + ", idgateID: " + idgateID + ", encDeregData: " + encDeregData);
			Log4jAP.log.info(apLogObj.getCompleteTxt());
		} else {
			Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][SvfSendDeregResponse] Error: "
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.ParameterError));
		...
		try {
			...
			Log4j.log.error("[" + sessionID + "][Version: " + svVerNo + "][SvfSendDeregResponse] DB Error occurred: "
					+ e.getMessage() + ", stacktrace: " + new Gson().toJson(e.getStackTrace()));

			apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo
					+ "][SvfSendDeregResponse] DB Error occurred: " + e.getMessage());
			apLogObj.setThrowable(e.getMessage());
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.DatabaseError));
		...
		if (chVO == null || "N".equals(chVO.getACTIVATE())) {
			Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][SvfSendDeregResponse] Response: "
					+ gts.common(ReturnCode.ChannelInvalidError));

			apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo + "][SvfSendDeregResponse] Response: "
					+ gts.common(ReturnCode.ChannelInvalidError));
			Log4jAP.log.info(apLogObj.getCompleteTxt());
...
//					.OutResult(gts.common(ReturnCode.ChannelInvalidError));
		...
		Log4j.log.info(
				"[" + sessionID + "][Version: " + svVerNo + "][SvfSendDeregResponse] Deregister iDGateID: " + idgateID);

		apLogObj.setMessage(
				"[" + sessionID + "][Version: " + svVerNo + "][SvfSendDeregResponse] Deregister iDGateID: " + idgateID);
		Log4jAP.log.info(apLogObj.getCompleteTxt());

		// flag this idgate id as disabled
		try {
			new MembersService(JNDI_Name, sessionID).disableOneMemberID(Long.parseLong(idgateID));
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.DatabaseError));
		...
		Log4j.log.trace("[" + sessionID + "][Version: " + svVerNo
				+ "][svfSendDeregResponse] Sending to FIDO svfSendDeregResponse API");
		apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo
				+ "][svfSendDeregResponse] Sending to FIDO svfSendDeregResponse API");
		Log4jAP.log.info(apLogObj.getCompleteTxt());
		...
		formparams.put("method", "svfSendDeregResponse");
		formparams.put("channel", channel);
		formparams.put("idgateID", idgateID);
		formparams.put("encDeregData", encDeregData);
...
//			retMsg = new Send2Remote().sendPost(FIDO_SV_IP, WSM_API_Key, gson.toJson(formparams));
			...
			Log4j.log.error(
					"[" + sessionID + "][Version: " + svVerNo + "][SvfSendDeregResponse] Connection to FIDO error: "
							+ e.getMessage() + ", stacktrace: " + new Gson().toJson(e.getStackTrace()));

		}

		Log4j.log.info(
				"[" + sessionID + "][Version: " + svVerNo + "][SvfSendDeregResponse] FIDO SV Response: " + retMsg);
		apLogObj.setMessage(
				"[" + sessionID + "][Version: " + svVerNo + "][SvfSendDeregResponse] FIDO SV Response: " + retMsg);
		Log4jAP.log.info(apLogObj.getCompleteTxt());
		...
		try {
			...
			Log4j.log.error("[" + sessionID + "][Version: " + svVerNo
					+ "][SvfSendDeregResponse] Unable to parse FIDO JSON msg: " + e.getMessage() + ", stacktrace: "
					+ new Gson().toJson(e.getStackTrace()));

			apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo
					+ "][SvfSendDeregResponse] Unable to parse FIDO JSON msg: " + e.getMessage());
			apLogObj.setThrowable(e.getMessage());
			Log4jAP.log.error(apLogObj.getCompleteTxt());
		...
		Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][SvfSendDeregResponse] Response: " + retMsg);

		apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo + "][SvfSendDeregResponse] Response: " + retMsg);
		Log4jAP.log.info(apLogObj.getCompleteTxt());
