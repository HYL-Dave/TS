...
/**
 * Servlet implementation class SvfSetAuthType
 */
@RestController
// @WebServlet(description = "設定其他驗證方式", urlPatterns = { "/svfSetAuthType" })
public class SvfSetAuthType extends BaseServlet {

	@Autowired
	WSMServlet wsmServlet;

	@PostMapping("/svfSetAuthType")
	protected String doPost(@RequestBody Object reqBody, HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		inLogObj.setClazz("com.WSM.controller.SvfSetAuthType");
		apLogObj.setTraceID(sessionID);
		apLogObj.setClazz("com.WSM.controller.SvfSetAuthType");
...
//				+ ", Err_msg_path: " + Err_msg_path);
		...
		Log4j.log.debug(
				"[" + sessionID + "][Version: " + svVerNo + "][SvfSetAuthType] API key in header: " + headerApiKey);
		apLogObj.setMessage(
				"[" + sessionID + "][Version: " + svVerNo + "][SvfSetAuthType] API key in header: " + headerApiKey);
		...
		if (headerApiKey == null || !WSM_API_Key.equals(headerApiKey)) {
			Log4j.log.debug("[" + sessionID + "][Version: " + svVerNo
					+ "][SvfSetAuthType] Reject client request for invalid API key");
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.APIKeyInvalid));
		...
		try {
			...
			Log4j.log.fatal("[" + sessionID + "][Version: " + svVerNo
					+ "][SvfSetAuthType] Error occurred while reading body: " + e.getMessage());
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.InternalError));
		...
		Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][SvfSetAuthType] Msg from body: " + jsonString);

		apLogObj.setMessage(
				"[" + sessionID + "][Version: " + svVerNo + "][SvfSetAuthType] Msg from body: " + jsonString);
		...
		try {
			...
			Log4j.log.error("[" + sessionID + "][Version: " + svVerNo
					+ "][SvfSetAuthType] Unable to parse client JSON msg: " + e.getMessage() + ", stacktrace: "
					+ new Gson().toJson(e.getStackTrace()) + ", Response: " + gts.common(ReturnCode.JsonParseError));

			apLogObj.setMessage(
					"[" + sessionID + "][Version: " + svVerNo + "][SvfSetAuthType] Error occurred while reading body: "
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.JsonParseError));
		...
		// log locale
		Log4j.log.trace("[" + sessionID + "][Version: " + svVerNo + "][SvfSetAuthType] Locale: " + req.getLocale());

		apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo + "][SvfSetAuthType] Locale: " + req.getLocale());
		...
		if (!map.containsKey("Error")) {
			...
			Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][SvfSetAuthType] channel: " + channel
					+ ", idgateID: " + idgateID + ", encSettingData: " + encSettingData);

			apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo + "][SvfSetAuthType] channel: " + channel
					+ ", idgateID: " + idgateID + ", encSettingData: " + encSettingData);
			Log4jAP.log.info(apLogObj.getCompleteTxt());

		} else {
			Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][SvfSetAuthType] Error: " + map.get("Error")
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.ParameterError));
		...
		try {
			...
			Log4j.log.error("[" + sessionID + "][Version: " + svVerNo + "][SvfSetAuthType] DB Error occurred: "
					+ e.getMessage() + ", stacktrace: " + new Gson().toJson(e.getStackTrace()));

			apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo + "][SvfSetAuthType] DB Error occurred: "
					+ e.getMessage());
			apLogObj.setThrowable(e.getMessage());
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.DatabaseError));
		...
		if (chVO == null || "N".equals(chVO.getACTIVATE())) {
			Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][SvfSetAuthType] Response: "
					+ gts.common(ReturnCode.ChannelInvalidError));

			apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo + "][SvfSetAuthType] Response: "
					+ gts.common(ReturnCode.ChannelInvalidError));
			Log4jAP.log.info(apLogObj.getCompleteTxt());
...
//					.OutResult(gts.common(ReturnCode.ChannelInvalidError));
		...
		Log4j.log.trace(
				"[" + sessionID + "][Version: " + svVerNo + "][svfSetAuthType] Sending to FIDO svfSetAuthType API");
		apLogObj.setMessage(
				"[" + sessionID + "][Version: " + svVerNo + "][svfSetAuthType] Sending to FIDO svfSetAuthType API");
		Log4jAP.log.info(apLogObj.getCompleteTxt());
		...
		formparams.put("method", "svfSetAuthType");
...
//			retMsg = new Send2Remote().sendPost(FIDO_SV_IP, WSM_API_Key, gson.toJson(formparams));
			...
			Log4j.log.error("[" + sessionID + "][Version: " + svVerNo + "][SvfSetAuthType] Connection to FIDO error: "
					+ e.getMessage() + ", stacktrace: " + new Gson().toJson(e.getStackTrace()));

			apLogObj.setMessage(
					"[" + sessionID + "][Version: " + svVerNo + "][SvfSetAuthType] Connection to FIDO error: "
							+ e.getMessage() + ", stacktrace: " + new Gson().toJson(e.getStackTrace()));
			apLogObj.setThrowable(e.getMessage());
			Log4jAP.log.error(apLogObj.getCompleteTxt());
...
//			new OutputHandler(res, "application/json; charset=UTF-8").OutResult(gts.common(ReturnCode.FidoSVError));
		...
		Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][SvfSetAuthType] FIDO SV Response: " + retMsg);

		apLogObj.setMessage(
				"[" + sessionID + "][Version: " + svVerNo + "][SvfSetAuthType] FIDO SV Response: " + retMsg);
		Log4jAP.log.info(apLogObj.getCompleteTxt());
		...
		if (map.containsKey("Error")) {
			Log4j.log.debug("[" + sessionID + "][Version: " + svVerNo + "][SvfSetAuthType] Error: " + map.get("Error"));
		...
		try {
			...
			Log4j.log.error(
					"[" + sessionID + "][Version: " + svVerNo + "][SvfSetAuthType] Unable to parse FIDO JSON msg: "
							+ e.getMessage() + ", stacktrace: " + new Gson().toJson(e.getStackTrace()));

			apLogObj.setMessage("[" + sessionID + "][Version: " + svVerNo
					+ "][SvfSetAuthType] Unable to parse FIDO JSON msg: " + e.getMessage());
			apLogObj.setThrowable(e.getMessage());
			Log4jAP.log.error(apLogObj.getCompleteTxt());
		...
		Log4j.log.info("[" + sessionID + "][Version: " + svVerNo + "][SvfSetAuthType] Response: " + retMsg);
