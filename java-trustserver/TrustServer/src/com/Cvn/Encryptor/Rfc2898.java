...
public class Rfc2898 {
     ...
     * @param baseKey The base-key used to derive the key.
     ...
     */
    public Rfc2898(byte[] baseKey, byte[] salt, int iterations) throws NoSuchAlgorithmException, InvalidKeyException {
        if ((salt == null) || (salt.length < 8)) { throw new InvalidKeyException("Salt must be 8 bytes or more."); }
        if (baseKey == null) { throw new InvalidKeyException("Password cannot be null."); }
        this._salt = salt.clone();
        this._iterationCount = iterations;
        this._hmacSha1 = Mac.getInstance("HmacSHA1");
        this._hmacSha1.init(new SecretKeySpec(baseKey, "HmacSHA1"));
    }
     ...
     */
    public Rfc2898(String baseKey, byte[] salt, int iterations) throws InvalidKeyException, NoSuchAlgorithmException, UnsupportedEncodingException  {
        this(baseKey.getBytes("UTF8"), salt, iterations);
    }
     ...
     * @throws UnsupportedEncodingException UTF-8 encoding is not supported. 
     */
    public Rfc2898(String baseKey, byte[] salt) throws NoSuchAlgorithmException, InvalidKeyException, UnsupportedEncodingException {
        this(baseKey, salt, 0x3e8);
    }
